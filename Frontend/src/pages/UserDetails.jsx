...
const UserDetails = () => {
  ...
  const handlePreviousBooking = async () => {
    try {
      const response = await fetch(`https://majestic-ganges-inn.onrender.com/api/v1/info/previousbookingdetails`, {
        method: "POST",
...
const handleRestaurantPreviousBooking = async () => {
  try {
    const response = await fetch(`https://majestic-ganges-inn.onrender.com/api/v1/info/previousrestaurantbooking`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: user.user._id }),
    });
...
const handleHallPreviousBooking = async () => {
  try {
    const response = await fetch(`https://majestic-ganges-inn.onrender.com/api/v1/info/previoushallbooking`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: user.user._id }),
    });
