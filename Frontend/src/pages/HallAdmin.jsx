...
const HallAdmin = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch("https://magestic-ganges-inn-backend.onrender.com/api/v1/info/fromtohallbooking", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        console.error("Error fetching hall bookings:", await response.text());
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto p-8 my-10 bg-white rounded-lg shadow-xl">
        <h1 className="text-2xl font-bold mb-4">Search Hall Bookings by Date</h1>
        <div className="mb-6">
        ...
        <div className="mt-8">
          {bookings.length === 0 ? (
            <p className="text-gray-500">
              No hall bookings found for the selected date.
            </p>
          ) : (
            <ul className="space-y-4">
              {bookings.map((booking, index) => (
                <li key={index} className="p-4 border rounded shadow">
                  <p>
                    <strong>Event Name:</strong> {booking.eventName || "N/A"}
                  </p>
                  <p>
                    <strong>Method:</strong> {booking.Method || "N/A"}
                  </p>
                  <p>
